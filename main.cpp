...
public:
  static void Awaken_Start() {
    sleep(1);
    cout << "Открыв глаза, ты продолжаешь видеть темноту и лишь твоё \n"
...
public:
  ...
  void Stat_Explanation(Player *&character) {
    system("cls");
    cout << "-Теперь глянь-ка сюда." << endl;
    sleep(1);
  ...
  void Fight_Train(Player *&character) {
    ...
    if (answer != "yes") { //выхд из метода, если не хош бучение проходить
      ...
      train =false;
	    ...
	    FurtherDialog();
	    system("cls");
	    cout << "Перед вами вдруг появляется манекен.\nОн выглядит достаточно "
	    ...
	    while (train == true) {
	      ...
	      if (ans == 1) {
					if(hpm<2){
						if(hpm==0){
	        		cout << "Вы атакуете манекен. Вам кажется, что это нелепо."<<endl;
							sleep(1);
							...
							cout << "Он жалобно смотрит на вас в ответ." << endl;
						 	...
						 	if(hpm==1){
						 		cout << "Продолжая атаковать манекен, на нём появляется всё"
						 				"\nбольше трещин."<<endl;
						 		sleep(1);
						 		cout << "Хорошим это явно не кончится" <<endl;
							 }
	      ...
	      if (ans == 2) {
					if(good_end<2){
		        cout << "Вы говорите манекену несколько хороших слов. Это странно,"
