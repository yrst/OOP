	...
	private:
		...
		void Fight_against(Player &character, mobclass &enemy_mob){
			int i,comand_cin;
				while((character.life==true) && (enemy_mob.current_health>0)){ //пока жив игрок или моб
					cout << "Что будете делать с "<< enemy_mob.name << endl << "1 - атаковать, 2 - характеристики, 4 - попытаться сбежать" <<endl;
					cin >> comand_cin;
					if(comand_cin==1){
					...
					if(comand_cin==2){
					  cout << "----------------"<<endl;
						...
						cout << "Уровень          " << character.current_lvl << "\t\t" << enemy_mob.lvl <<endl;
						cout << "Текущее здоровье " << character.current_health << "\t\t" << enemy_mob.current_health << endl;
		...
		void attack(Player &character, mobclass &enemy_mob){
			...
			if (reverse_attack > 50){
				...
				cout << "У вас осталось " << character.current_health << " хп" << endl;
				cout << "----------------"<<endl;
			}
			if (character.current_health<=0){
